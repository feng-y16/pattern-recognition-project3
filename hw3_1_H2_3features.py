...
def branch_and_bound_choosetwo(data1,data2,remainfeatures,ignorefeatures):#分支定界算法
    ...
    H_two[max_2]=H_two[max_2]-10000

    max_3=np.where(H_two==np.max(H_two))
    max3=tempfeatures_minus[list(max_3)[0][0]]
    max_3=list(max_3)[0][0]
    temp=H_two_decide(data1, data2, [max1,max2,max3])
    if temp<min_entrophy:
        min_entrophy=temp
        finalfeature=[max1,max2,max3]
    branch_and_bound_choosetwo(data1,data2,np.delete(remainfeatures,max_1),ignorefeatures+[max2]+[max3])
    branch_and_bound_choosetwo(data1,data2,np.delete(remainfeatures,max_2),ignorefeatures+[max3])
    branch_and_bound_choosetwo(data1,data2,np.delete(remainfeatures,max_3),ignorefeatures)
#######################################################################################################
min_entrophy=10000
finalfeature=[0,1]
