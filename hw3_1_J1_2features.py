...
#    for j in range(0,10):
#        if j!=i:
#            temp=J_one_decide(data1,data2,[i,j])
...
def branch_and_bound_choosetwo(data1,data2,remainfeatures,ignorefeatures):#分支定界算法
    ...
    if J_one_decide(data1, data2, remainfeatures)<max_distinguish:
        ...
        temp=J_one_decide(data1, data2, [min1,min2])
        if temp>max_distinguish:
            max_distinguish=temp;
            finalfeature=[min1,min2]
        branch_and_bound_choosetwo(data1,data2,np.delete(remainfeatures,min_1),ignorefeatures)
        branch_and_bound_choosetwo(data1,data2,np.delete(remainfeatures,min_2),ignorefeatures+[min1])
#######################################################################################################
max_distinguish=-1
finalfeature=[0,1]
